...
for files in os.listdir(directory):
    ...
    if files.endswith(".png"):
        #Train the boxfiles
        rename = 'mv '+files+' '+language+'.'+fontname+'.exp'+str(count)+'.png'
        os.system(rename)
        command='tesseract itk.'+fontname+'.exp'+str(count)+'.png itk.'+fontname+'.exp'+str(count)+' nobatch box.train.stderr'
        print command   
        os.system(command)
        count = count + 1
